...
def alinea(timeStamp):
    ...
    # 临界密度最佳值：55，55，80，90
    critical_density1 = 60
    critical_density2 = 60
    critical_density3 = 85
    critical_density4 = 95
    # 最佳Kr = 0.005
    Kr = 0.007
    lastNum = 0
    lastMRList1 = [0.6]
    ...
    while step < timeStamp:
        ...
        if step % ramp_control_cycle == 0:
            # 先算四个匝道的调节率和绿灯时间，然后按照取值范围得到实际绿灯时间列表
            metering_rate1 = lastMRList1[lastNum] + Kr * (critical_density1 - den1)
            green_time1 = metering_rate1 * ramp_signal_cycle

            metering_rate2 = lastMRList2[lastNum] + Kr * (critical_density2 - den2)
            green_time2 = metering_rate2 * ramp_signal_cycle

            metering_rate3 = lastMRList3[lastNum] + Kr * (critical_density3 - den3)
            green_time3 = metering_rate3 * ramp_signal_cycle

            metering_rate4 = lastMRList4[lastNum] + Kr * (critical_density4 - den4)
            green_time4 = metering_rate4 * ramp_signal_cycle

            green_time_list = [green_time1, green_time2, green_time3, green_time4]
            ...
            if metering_rate1 < 0:
                lastMRList1.append(0)
            else:
                lastMRList1.append(metering_rate1)

            if metering_rate2 < 0:
                lastMRList2.append(0)
            else:
                lastMRList2.append(metering_rate2)

            if metering_rate3 < 0:
                lastMRList3.append(0)
            else:
                lastMRList3.append(metering_rate3)

            if metering_rate4 < 0:
                lastMRList4.append(0)
            else:
                lastMRList4.append(metering_rate4)
    ...
    vel_df.to_csv('alineaSpeed.csv', encoding='utf-8', index=True)
    den_df = pd.DataFrame.from_dict(denDict, orient='index').transpose()
    den_df.to_csv('alineaDensity.csv', encoding='utf-8', index=True)
...
#
#
# ttt, twt, que, flow, den = alinea(10800)
# print('ALINEA旅行时间：', ttt, '等待时间', twt, '排队长度：', que, '通过流量：', flow, '瓶颈密度：', den)


alinea(10800)

# ttt, twt, que, flow, den, pd = alinea(10800)
# print(ttt, twt, que, flow, den)
# for p in pd:
#     print(p)

# plt.figure(figsize=(20, 10), dpi=160)
# plt.title("all queue figure", fontsize=22)
# plt.plot(ql, color='tab:red')
# plt.show()
# plt.figure(figsize=(20, 10), dpi=160)
# plt.title("travel time", fontsize=22)
# plt.plot(tl, color='tab:blue')
# plt.show()
# plt.figure(figsize=(20, 10), dpi=160)
# plt.title("waiting time", fontsize=22)
# plt.plot(wl, color='tab:green')
# plt.show()


# result1 = {}
# result2 = {}
# for crd1 in np.arange(45, 71, 5):
#     for crd2 in np.arange(55, 81, 5):
#         result1[crd1, crd2] = alinea(10800,  crd1, crd2, 80, 90)

# for crd3 in np.arange(70, 95, 5):
#     for crd4 in np.arange(80, 101, 5):
#         result2[crd3, crd4] = alinea(10800,  55, 55, crd3, crd4)

# for item in result1:
#     print("第一第二个临界密度为：", item, "结果：", result1[item])

# for item1 in result2:
#     print("第三第四个临界密度为：", item1, "结果：", result2[item1])



# result = {}
# for kr in np.arange(0.001, 0.005, 0.001):
#     result[kr] = alinea(10800, kr)
#
# for item in result:
#     print("kr：", item, "结果：", result[item])