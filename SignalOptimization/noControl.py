...
def no_control(timeStamp):
    ...
    while step < timeStamp:
        # 匝道无控制，全绿
        traci.trafficlight.setRedYellowGreenState("rl1", "GG")
        traci.trafficlight.setRedYellowGreenState("rl2", "GG")
        traci.trafficlight.setRedYellowGreenState("rl3", "GG")
        traci.trafficlight.setRedYellowGreenState("rl4", "GG")
    ...
    vel_df.to_csv('noSpeed.csv', encoding='utf-8', index=True)
    den_df = pd.DataFrame.from_dict(denDict, orient='index').transpose()
    den_df.to_csv('noDensity.csv', encoding='utf-8', index=True)
    ...
    df_result.to_csv('result.csv', encoding='utf-8', index=True)

    tttDict = {'nocontrol': travelTime}
    df_ttt = pd.DataFrame.from_dict(tttDict, orient='index').transpose()
    df_ttt.to_csv('TTT.csv', encoding='utf-8', index=False, header=True)
...
#
#
# ttt, twt, que, flow, den = no_control(10800)
# print('无控制旅行时间：', ttt, '等待时间', twt, '排队长度：', que, '通过流量', flow, '瓶颈密度：', den)
